...
func (s *Server) AddPeer(url, id, token string) {
	...
	GetLogger().Infof("Adding peer %s, %s", url, id)
...
func (s *Server) RemovePeer(id string) {
	...
	if p, ok := s.peers[id]; ok {
		GetLogger().Infof("Removing peer %s", id)
		p.cancel()
	}
	delete(s.peers, id)
}
...
outer:
	for {
		...
		if err != nil {
			GetLogger().Errorf("Failed to connect to peer %s [local ID=%s]: %v", p.url, s.PeerID, err)
		...
		if err != nil {
			GetLogger().Errorf("Failed to serve peer connection %s: %v", p.id, err)
		}
